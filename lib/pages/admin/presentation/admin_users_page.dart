...
class _AdminUsersPageState extends State<AdminUsersPage> {
  ...
  void _showStatusChangeDialog(UserModel user, String targetStatus) {
    _reasonController.clear();
    String title = 'Suspend Account';
    if (targetStatus == 'blocked') title = 'Block Account';
    if (targetStatus == 'active') title = 'Re-activate Account';
    ...
    showDialog(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text('User: @${user.username ?? user.name}', style: CustomTextStyle.size14W600(color: AppColors.grey300)),
            const SizedBox(height: AppSize.spacingM),
            Text('Reason for status change:', style: CustomTextStyle.size14W500(color: AppColors.grey300)),
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              final reasonText = _reasonController.text.trim().isEmpty ? 'Admin status update' : _reasonController.text.trim();
              ...
              if (mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text('User status updated to $targetStatus'), backgroundColor: AppColors.success),
                );
              }
            },
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: AppColors.grey900,
        elevation: 0,
        title: Text('User Moderation', style: CustomTextStyle.size18W600(color: AppColors.white100)),
        leading: IconButton(
            ...
            child: StreamBuilder<QuerySnapshot>(
              ...
              builder: (context, snapshot) {
                ...
                if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
                  return Center(
                    child: Text('No users found.', style: CustomTextStyle.size14W400(color: AppColors.grey400)),
                  );
                }
                          ...
                          Row(
                            ...
                            children: [
                              if (user.accountStatus != 'active')
                                OutlinedButton(
                                  onPressed: () => _showStatusChangeDialog(user, 'active'),
                                  style: OutlinedButton.styleFrom(side: const BorderSide(color: AppColors.success)),
                                  child: Text('Activate', style: CustomTextStyle.size12W600(color: AppColors.success)),
                                ),
                              if (user.accountStatus != 'suspended') ...[
                                const SizedBox(width: 6),
                                OutlinedButton(
                                  onPressed: () => _showStatusChangeDialog(user, 'suspended'),
                                  style: OutlinedButton.styleFrom(side: const BorderSide(color: Colors.purpleAccent)),
                                  child: Text('Suspend', style: CustomTextStyle.size12W600(color: Colors.purpleAccent)),
                                ),
                              ],
                              if (user.accountStatus != 'blocked') ...[
                                const SizedBox(width: 6),
                                OutlinedButton(
                                  onPressed: () => _showStatusChangeDialog(user, 'blocked'),
                                  style: OutlinedButton.styleFrom(side: const BorderSide(color: AppColors.error)),
                                  child: Text('Block', style: CustomTextStyle.size12W600(color: AppColors.error)),
                                ),
                              ],
                            ],
                          ),
